...
void DialogGUI::initUI()
{
	...
	bg = Sprite::create("res/APK/Dialog/bg.png");
	addChild(bg);
	...
	auto title = Sprite::create("res/APK/Dialog/title.png");
	bg->addChild(title);
	title->setPosition(bSize.width / 2, bSize.height - title->getContentSize().height/2);

	content = Text::create("", "res/APK/font_content.ttf", 25);
	...
	content->setTextColor(Color4B(203, 204, 206, 255));
	content->ignoreContentAdaptWithSize(false);
	content->setContentSize(CCSize(bSize.width*0.9f, bSize.height / 2));
	bg->addChild(content);

	btnOK = Button::create("res/APK/Dialog/ok.png", "res/APK/Dialog/ok.png");
	btnCancel = Button::create("res/APK/Dialog/cancel.png", "res/APK/Dialog/cancel.png");
	btnOK->setTag(0);
	btnCancel->setTag(1);
